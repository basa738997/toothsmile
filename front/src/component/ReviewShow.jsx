...
const ReviewList = () => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const response = await axios.get("http://localhost:5009/api/v1/review/all");
  ...
  return (
    <div className="container mx-auto px-4 py-10 font-serif ">
      <h3 className="text-2xl font-semibold mb-6 text-center">All Reviews</h3>
        ...
        {error ? (
          ...
          reviews.length > 0 ? (
            reviews.map((review, index) => (
              ...
              >
                <p className="font-bold text-[#3e2015] capitalize">{review.name}</p>
                <p className="text-yellow-500 pt-2">Rating: {review.rating}/5</p>
                <p className="text-[#3e2015] pt-4">{review.comment}</p>
                <p className="text-xs pt-1 text-gray-500">Posted on: {new Date(review.createdAt).toLocaleString()}</p>
              </div>
            ))
          ) : (
